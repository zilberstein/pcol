...
theorem pom_ge_iff_ge_fin {l : Type}
  ...
  let d : DSet (Lpo l) :=
    ⟨ Set.range (fun n ↦ (a.trunc n).val.permute (f n).heq.choose),
      by {
        constructor
        · intro α hα β hβ; sorry
  ...
  have hn (n : ℕ) : (f n).n = n := by
    ...
    | zero => rw [h₀]; rfl
    | succ n => sorry
  refine ⟨d.dSup, b, ?_, ?_, rfl⟩
  · refine DSet.dSup_le ?_; intro α h; unfold d at h
    obtain ⟨n, rfl⟩ := Set.mem_range.mp h
    rw [← hn n]
    refine (f n).le_b _ ?_  --(Exists.choose_spec _)
    sorry
